...
共有四类测量，分别为：
- 计数测量：计算单位时间内射入探测器的粒子数。其具体应用有：活度、中子注量测量、核反应截面等
- 能量测量：测量 (单位时间内) 各能量区间中射入探测器的粒子数。其具体应用有：带电粒子出射能谱、γ 能谱等
- 时间测量：测量各种时间间隔。其具体应用有：各种寿命测量、飞行时间法、扰动角关联
- 位置测量：一种是变换探测器角度的位置测量，一种是使用位置灵敏的探测器进行测量，还有一种是径迹测量。其具体应用有：角分布测量、背散射测量、高能物理等

**来源**：

- PPT 讲义《绪论 射线与物质相互作用(2)》P13
- 课程 第 1 次课，视频 1 约 03:01:58 ~ 03:14:53 处

**讨论**：

- 或许应该删去 "其具体应用有" 这部分，因为举例用词太简洁
...
(5) 不适合。应使用电荷灵敏前置放大器。因为半导体探测器时常需要调节的外加电压会影响其结电容乃至等效电容的大小，从而影响输出回路中形成的电压。故电压灵敏前放输出信号幅度不仅与带电粒子电荷有关，还与探测器所加偏压有关。而电荷灵敏前放的特点是输入电容极大，结电容和杂散电荷可以忽略，输出信号与偏压无关。因此半导体探测器适用电荷灵敏而非电压灵敏前放。
...
- (5) 的答案来自某位同学。原没有来源的答案 "电荷灵敏前置放大器噪声低，而电压灵敏放大器噪声高" 暂且删去。
------
...
![γ 射线的吸收曲线](https://github.com/nuclear9725/RadiationDetectorQuestionPool/blob/440e05f8b5c5f4d35ca03ec1fc5599c56b8fab4f/image/%CE%B3%E5%90%B8%E6%94%B6%E6%9B%B2%E7%BA%BF.png)
 ...
 对 α 粒子而言，半吸收厚度为 α 粒子计数降至原来的一半的吸收层厚度；而言对 γ 射线和单能电子而言，半吸收厚度指射线强度减半处的吸收层厚度。
...
![电离室的线性范围](https://github.com/nuclear9725/RadiationDetectorQuestionPool/blob/74778f67115acafd44217124c601572be14db4b5/image/%E7%94%B5%E7%A6%BB%E5%AE%A4%E7%9A%84%E7%BA%BF%E6%80%A7%E8%8C%83%E5%9B%B4.jpg)
...
- 为什么正比计数器和 G-M 计数器中央丝必须是阳极
  - 因为只有被阳极吸引的进入高电场区的电子才能倍增，而中央丝表面积小，一定电压下以中央丝为阳极能在其附近产生更高强度的电场。另一方面，即使令中央丝为阴极，正离子也不足以发生雪崩。
...
- G-M 计数器自猝息的机制
  - 在单原子或双原子气体中加入少量的猝熄气体，使计数管在放电后自行猝熄。
  - 根据所用猝熄气体，可将自猝熄分为机制略有区别的两类：
    ...
    - 卤素自猝熄
      - 吸收紫外光
      - 抑制正离子发射
      - 消除 Ne 原子压稳态妨碍电子增殖的作用
      - 提高电子脉冲，电子脉冲在负载电阻上的压降使电场减弱、放电终止
      - 卤素猝熄分子解离后会重新复合
- G-M 计数器的寿命
  - 概念
...
以碘化钠为比较对象。就谱形而言，NaI 有全能峰、康普顿坪、特征 X 射线峰、反散射峰等。
- 塑料闪烁体：碳氢化合物密度小、原子序数低， 0.662MeV γ  射线与之主要发生康普顿散射，只有康普顿连续谱，谱形也不如 NaI 丰富；发光效率也低，所以总计数不如 NaI 高。
- 氟化钡：光衰减时间最小，故探测效率高，总计数大。但是，它发光效率低，能量分辨率不高，所以全能峰不明显，峰总比低。
- 溴化镧：晶体通常小于 NaI，导致峰总比较小，但能量分辨率好于 NaI，具有更高峰康比，全能峰明显好于 NaI。其探测效率低于 NaI，因此谱形总计数较小。
...
- 为什么在一片硅片上（电阻率 1000Ωcm）的两个面上制备欧姆接触不能构成核辐射探测器？
  - 因为电阻率过低。如果在一片硅片的两个面上制备欧姆接触，在接触处不会产生载流子浓度的变化，不能形成没有自由载流子、对电导没有贡献的耗尽区。
...
- 闪烁体探测器
  - 优点：时间相应快、探测效率高
  - 缺点：能量分辨较低、价格较高
  - 适用场合：应用最广的核探测器类型。可测带电和中性粒子；多用于时间测量、计数测量，也可用于低水平放射性与能谱测量；适用于高温场合
- 气体探测器
  - 优点：结构简单，使用方便可靠，成本低，可做很大
  - 缺点：能量分辨不高，响应速度慢，阻止能力差
  - 适用场合：逐步被取代的传统类型，较多在工业上应用；新型气体探测器用于核物理、高能物理和宇宙射线探测
...
![G-M计数管的坪曲线](https://github.com/nuclear9725/RadiationDetectorQuestionPool/blob/a415de91947b6d7c4d5aa89153e799a2e63b33f6/image/G-M%E8%AE%A1%E6%95%B0%E7%AE%A1%E7%9A%84%E5%9D%AA%E6%9B%B2%E7%BA%BF.png)
...
- 1368 keV 和 2754 keV 处是 <img src="https://render.githubusercontent.com/render/math?math=\^{24}\text{Na}"> 的全能峰，入射光子能量全部被探测器吸收形成的。
- 最左端是 1368 keV 全能峰对应的康普顿坪。一些入射光子与探测器发生康普顿散射后逃逸，只有部分能量被探测器吸收，反冲电子的能量随散射角连续变化，形成康普顿坪。
- 康普顿坪右端是康普顿边缘。其对应康普顿反冲电子最大能量。
- 1368 keV 往右两个峰依次是 2754 keV 全能峰对应的双逃逸峰和单逃逸峰。入射光子与探测器发生的电子对效应产生的正电子湮灭时产生的两个能量为 0.511 MeV 的光子，两个湮灭光子全都逃逸对应双逃逸峰，一个逃逸另一个被吸收对应单逃逸峰。
- 2754 keV 全能峰左边为与其对应的康普顿坪和康普顿边缘。
...
热释光磷光体受辐射照射后，其中电子可能被激发而落入不同的陷阱能级。常温下这些电子跳不到导带。辐照后的晶体被加热时，被俘获的电子获得能量。若此能量足够高，电子能挣脱陷阱能级的束缚而重新被激发到导带，由导带跳回满带，将激发能以光的形式辐射，发射的这种光被称为热释光。加热放光的光子数与陷阱中释放的电子数成正比，而释放的电子数与物体最初吸收的辐射能量成正比。因此，可以通过测量总光子数来探测各种核辐射。
